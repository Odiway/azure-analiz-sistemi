...
import {
  ...
  Users, ListOrdered, Timer, ChevronDown, X,
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  async function handleExit(serverName: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) toast(data.error, 'warn');
      else toast('Çıkış başarılı!', 'success');
      await fetchStatus();
    } catch { toast('Bağlantı hatası', 'warn'); }
              ...
              className={`relative overflow-hidden rounded-3xl transition-all duration-500 ${
                occupied
                  ? isMe
                    ...
                    : overtime
                      ? 'ring-2 ring-orange-400 shadow-2xl shadow-orange-100'
                      : 'ring-2 ring-red-400 shadow-2xl shadow-red-100'
                  : 'ring-1 ring-green-300 shadow-xl shadow-green-50 hover:shadow-2xl'
              }`}
            >
              {/* Top bar */}
              <div className={`h-1.5 ${
                occupied
                  ? isMe ? 'bg-gradient-to-r from-azure-400 to-blue-600'
                    : overtime ? 'bg-gradient-to-r from-orange-400 to-amber-500'
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className={`relative w-12 h-12 rounded-2xl flex items-center justify-center ${
                      occupied ? (isMe ? 'bg-azure-500' : overtime ? 'bg-orange-500' : 'bg-red-500') : 'bg-emerald-500'
                    }`}>
                      <Monitor className="w-6 h-6 text-white" />
                      {occupied && <div className={`absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-white ${isMe ? 'bg-azure-400' : 'bg-red-400'} animate-pulse`} />}
                    </div>
                    <div>
                      ...
                      <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2 py-0.5 rounded-full ${
                        occupied
                          ? isMe ? 'bg-azure-100 text-azure-700'
                            : overtime ? 'bg-orange-100 text-orange-700'
                            : 'bg-red-100 text-red-700'
                          : 'bg-green-100 text-green-700'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${occupied ? (isMe ? 'bg-azure-500' : overtime ? 'bg-orange-500' : 'bg-red-500') : 'bg-green-500'}`} />
                        {occupied ? (isMe ? 'İçeridesiniz' : overtime ? 'Süre Aşımı' : 'Meşgul') : 'Müsait'}
                      </span>
                    </div>
                  </div>
                        ...
                        <div className="space-y-1">
                          {q.map((item) => (
                            <div key={item.userId} className={`flex items-center gap-2 text-xs px-2.5 py-1.5 rounded-lg ${item.userId === myId ? 'bg-amber-200/70 font-bold text-amber-900' : 'bg-amber-100/50 text-amber-800'}`}>
                              <span className="w-5 h-5 rounded-full bg-amber-200 flex items-center justify-center text-[10px] font-bold text-amber-700">
                                {item.position}
                              </span>
